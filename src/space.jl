...
const _cache = Dict{DataType, Space}()
...
isempty(s::Space) = length(elements(s)) == 0
isempty(s::Type{<:Space}) = length(elements(s)) == 0
...
function _set(s::Symbol)
    quote
        struct $(esc(s)) <: BasicSet
            ...
            function $(esc(s))()
                get!( () -> new( $(QuoteNode(s)), Objects{$(esc(s))}() ), _cache, $(esc(s)) )
            end
        end
    end
end
...
∩(T1::Type{<:Space}, T2::Type{<:Space}) = SetIntersection{Tuple{T1,T2}}
∩(::Type{SetIntersection{T1}}, T2::Type{<:Space}) where T1 = SetIntersection{Tuple{fieldtypes(T1)...,T2}}
∩(T1::Type{<:Space}, ::Type{SetIntersection{T2}}) where T2 = SetIntersection{Tuple{T1,fieldtypes(T2)...}}
∩(::Type{SetIntersection{T1}}, ::Type{SetIntersection{T2}}) where {T1, T2} = SetIntersection{Tuple{fieldtypes(T1)...,fieldtypes(T2)...}}
...
∪(T1::Type{<:Space}, T2::Type{<:Space}) = SetUnion{Tuple{T1,T2}}
∪(::Type{SetUnion{T1}}, T2::Type{<:Space}) where T1 = SetUnion{Tuple{fieldtypes(T1)...,T2}}
∪(T1::Type{<:Space}, ::Type{SetUnion{T2}}) where T2 = SetUnion{Tuple{T1,fieldtypes(T2)...}}
∪(::Type{SetUnion{T1}}, ::Type{SetUnion{T2}}) where {T1, T2} = SetUnion{Tuple{fieldtypes(T1)...,fieldtypes(T2)...}}
...
struct CartesianProduct{T<:Tuple{Vararg{Space}}} <: Space
    ...
    # elements::Set{Tuple{Vararg{Object}}}

    function CartesianProduct{T}() where {T<:Tuple{Vararg{Space}}}
        get!(_cache, CartesianProduct{T}) do
            # new{T}( Objects{CartesianProduct{T}}() )
            # new{T}( Set{cartesian_to_tuple(T)}() )
...
struct TupleDecomposition{T<:Tuple{Vararg{Space}}} <: Decomposition{CartesianProduct{T}}
    ...
    function TupleDecomposition{T}(val::Vararg{Object}) where T
...
convert(::Object{CartesianProduct{T}}, x::Tuple{Vararg{Object}}) where T = TupleDecomposition{T}(x)
...
×(T1::Type{<:Space}, T2::Type{<:Space}) = CartesianProduct{Tuple{T1,T2}}
×(::Type{CartesianProduct{T1}}, T2::Type{<:Space}) where T1 = CartesianProduct{Tuple{fieldtypes(T1)...,T2}}
×(T1::Type{<:Space}, ::Type{CartesianProduct{T2}}) where T2 = CartesianProduct{Tuple{T1,fieldtypes(T2)...}}
×(::Type{CartesianProduct{T1}}, ::Type{CartesianProduct{T2}}) where {T1, T2} = CartesianProduct{Tuple{fieldtypes(T1)...,fieldtypes(T2)...}}
...
elements(T::Type{<:CartesianProduct}, ind::Int) = Set( x[ind] for x ∈ elements(T) )
...
→(T1::Type{<:Space}, T2::Type{<:Space}) = FunctionSpace{T1,T2}

function (f::Object{FunctionSpace{X,Y}})(x::Object{X}) where {X<:Space, Y<:Space}
    if x ∈ inputs(f)
        value(first(filter( t -> value(t)[1] === x, elements(graph(f)))))[2]
    else
        y = sample(Y, Symbol(label(f), "(", label(x), ")"))
        push!(graph(f), TupleDecomposition{Tuple{X,Y}}(x,y))
        y
    end
end
...
function (f::Object{OperatorSpace{X,Y}})(x::Object{X}) where {X<:Space, Y<:Space}
    y = sample(Y, Symbol(label(f), "(", label(x), ")"))
    push!(graph(f), TupleDecomposition{Tuple{X,Y}}(x,y))
    y
end
