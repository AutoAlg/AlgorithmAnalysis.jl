export maximize, base_spaces, dependencies, rootcause, interpolation
export optimization_variable_dictionary, optimization_variable, optimization_constraint
...
#     if hastrait(s, Product)
#         Spaces(spaces(s))
#     # elseif hastrait(s, Map)
#     #     Spaces([domain(s), codomain(s), s])
#     elseif hastrait(s, Subset)
#         Spaces([parent(s)])
#     else
#         Spaces()
#     end
...
# rootcause(c::Term) = filter(related(c)) do s

#     if !(s isa Space) || implementable(s)
#         return false
#     end

#     # Get its children
#     children = dependencies(s)

#     # It is a ROOT cause only if ALL children are valid (or if it has no children)
#     # If any child is broken, we blame the child instead of 's'
#     isempty(children) || all(c -> implementable(c), children)
# end

# base_spaces(t::Term) = base_spaces(Terms([t]))

# function base_spaces(todo::Set{Space})
...
#         if !isnothing(get(s, Product))
#             union!(todo, spaces(s))

#         elseif !isnothing(get(s, GraphOf))
#             push!(todo, space(graphof(s)))
            
#         elseif !isnothing(get(s, Subset))
#             push!(todo, parent(s))
            
#         # elseif !isnothing(get(s, Map))
#         #     push!(todo, domain(s))
#         #     push!(todo, codomain(s))
