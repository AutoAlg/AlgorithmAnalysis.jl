...
export Trait, Traits, TraitSet, traits, add_trait!, remove_trait!, has_trait, get_trait
export components, domain, codomain, @set, Functional, Invertible


############################################################################################

const _CACHE = IdDict{Any, Space}()
...
add_trait!(x::Object, t::Trait) = push!(traits(x), t)
remove_trait!(x::Object, t::Trait) = delete!(traits(x), t)
has_trait(x, T::Type{<:Trait}) = any(t -> typeof(t) <: T, traits(x))

function get_trait(x, T::Type{<:Trait})
    for trait in traits(x)
        typeof(trait) <: T && return trait
    end
    return nothing
end
...
############################################################################################
struct TraitSet <: Space
    label::Label
    elements::Set{Object}
    traits::Traits

    TraitSet(label::Label = missing) = new( label, Set{Object}(), Traits() )
    
    function TraitSet(traits::Trait...)
        new( missing, Set{Object}(), Traits(traits) )
    end
end
...
function _set(s::Symbol)
    quote
        $(esc(s)) = TraitSet($(QuoteNode(s))); nothing
    end
end
...
struct IntersectionTrait <: Trait
    components::Tuple{Vararg{Space}}

    IntersectionTrait(components::Space...) = new(components)
end

components(t::IntersectionTrait) = t.components

struct UnionTrait <: Trait
    components::Tuple{Vararg{Space}}

    UnionTrait(components::Space...) = new(components)
end

components(t::UnionTrait) = t.components

struct CartesianProductTrait <: Trait
    components::Tuple{Vararg{Space}}

    CartesianProductTrait(components::Space...) = new(components)
end

components(t::CartesianProductTrait) = t.components

struct Operator <: Trait
    domain::Space
    codomain::Space
    graph::Dict{Object, TraitSet}

    Operator(X::Space, Y::Space) = new( X, Y, Dict{Object, TraitSet}() )
end

domain(t::Operator) = t.domain
codomain(t::Operator) = t.codomain
graph(t::Operator) = t.graph

struct Functional <: Trait end
struct Invertible <: Trait end

function ⇒(X::TraitSet, Y::TraitSet)
    get!(_CACHE, (:⇒,X,Y)) do
        TraitSet( Operator(X, Y) )
    end
end

function →(X::TraitSet, Y::TraitSet)
    get!(_CACHE, (:→,X,Y)) do
        TraitSet( Operator(X, Y), Functional() )
    end
end

function ∩(X::TraitSet, Y::TraitSet)
    t1 = has_trait(X, IntersectionTrait) ? components(get_trait(X, IntersectionTrait)) : (X,)
    t2 = has_trait(Y, IntersectionTrait) ? components(get_trait(Y, IntersectionTrait)) : (Y,)
    T = sort(unique(collect(t1 ∪ t2)), by = x -> string(x))
    TraitSet( IntersectionTrait(T...) )
end

function ∪(X::TraitSet, Y::TraitSet)
    t1 = has_trait(X, UnionTrait) ? components(get_trait(X, UnionTrait)) : (X,)
    t2 = has_trait(Y, UnionTrait) ? components(get_trait(Y, UnionTrait)) : (Y,)
    T = sort(unique(collect(t1 ∪ t2)), by = x -> string(x))
    TraitSet( UnionTrait(T...) )
end

function ×(X::TraitSet, Y::TraitSet)
    t1 = has_trait(X, CartesianProductTrait) ? components(get_trait(X, CartesianProductTrait)) : (X,)
    t2 = has_trait(Y, CartesianProductTrait) ? components(get_trait(Y, CartesianProductTrait)) : (Y,)
    T = (t1...,t2...)
    TraitSet( CartesianProductTrait(T...) )
end

function (S::TraitSet)(x::Object)
    if Operator ∉ typeof.(structures(S))
        error("Set $S has no operator trait")
    end
    T = get_trait(S, Operator)
    get!(T.graph, x) do
        TraitSet()
    end
end
...
show(io::IO, x::Operator) = print(io, label(domain(x)), " ⇒ ", label(codomain(x)))
show(io::IO, x::IntersectionTrait) = print(io, join(components(x), " ∩ "))
show(io::IO, x::UnionTrait) = print(io, join(components(x), " ∪ "))
show(io::IO, x::CartesianProductTrait) = print(io, join(components(x), " × "))
...
#     structures::Dict{Object{OperatorSpace{X,Y}}, Structures}
...
#             Dict{Object{OperatorSpace{X,Y}}, Structures}()
#         )
#     end
# end
