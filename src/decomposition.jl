...
"""
mutable struct OrderedSet{T} <: Decomposition{T}
    label::String
    args::NTuple{N, T} where N
    ...
    OrderedSet(x::NTuple{N,T}) where {N,T} = new{T}(
        "",
        x,
        Constraints(),
        Operators(),
