...
export domain, codomain, inputs, outputs

import Base.push!, Base.length, Base.inv, Base.zeros, Base.∈, Base.+, Base.∘
import Base.show, Base.∩, Base.Generator, Base.iterate, Base.==, Base.-, Base.*
...
mutable struct Relation{X,Y}
  ...
  Relation{X,Y}(label::String = "") where {X,Y} = new(Set{Pair{X,Y}}(), label)
