...
- Other suboracles (accessed via the `adjoint`) are given intuitive labels. For example:
    - the transpose of a linear map with label `A` is `Aᵀ`
...
```julia-repl
julia> @autolabel a = R()  # define and label a scalar
julia> label(a)            # "a"
julia> @autolabel begin
...
function _autolabel(ex::Expr)
    # if the expression is not an assignment, then just evaluate it
    if ex.head ≠ :(=)
        return :($(esc(ex)))
    end
    
    # if the expression is an assignment, then evaluate it and label the new variable
