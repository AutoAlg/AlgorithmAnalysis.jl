...
function smooth_strongly_convex_interpolation_is_applicable(opt::BasicSymbolic{Optimization})::Bool
    predicate = c -> isequal(symtype(c), Constraint) && iscall(c) && isequal(operation(c), smooth_strongly_convex)

    return !isempty(find_nodes(predicate, opt))
end

function smooth_strongly_convex_interpolation(opt::BasicSymbolic{Optimization})::BasicSymbolic{Optimization}
    predicate = (node::Any) -> isequal(symtype(node), Constraint) && iscall(node) && isequal(operation(node), smooth_strongly_convex)
    constraints::Set{Any} = find_nodes(predicate, opt)
    ...
    for constraint_node::BasicSymbolic{<:Constraint} ∈ constraints
        f::BasicSymbolic = arguments(constraint_node)[1]
        μ::BasicSymbolic = arguments(constraint_node)[2]
        L::BasicSymbolic = arguments(constraint_node)[3]
        topt = smooth_strongly_convex_interpolation(topt, f, μ, L)
    end
