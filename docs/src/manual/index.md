...
Oracle
  ...
  Traits: Linear()
```
Instead of viewing this as a row vector, this is a linear functional (a function from the vector space to its underlying scalar field) over the vector space `Rⁿ`. Once again, it is given an intuitive label based on the label of the vector `u`. In addition to a label, the oracle has a set of traits, in this case the `Linear()` property. With this understanding, the inner product is applying the linear functional `u'` to the vector `v`, which results in the scalar `u'*v`.
